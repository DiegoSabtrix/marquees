...
export async function POST(request:Request) {
  ...
  if(!settings) return Response.json({error:"Online payment is not configured yet"},{status:503});
  const mode=(settings.active_mode||"test") as "test"|"live";
  const encrypted=settings[`${mode}_secret_key_encrypted`] as string|undefined;
  if(!encrypted) return Response.json({error:`Stripe ${mode} mode is not configured`},{status:503});
  ...
  catch { await failPaymentAttempt(attemptId,"Stripe network error"); return Response.json({error:"Stripe is temporarily unavailable"},{status:502}); }
  const result=await stripe.json() as {id?:string;url?:string;error?:{message?:string}};
